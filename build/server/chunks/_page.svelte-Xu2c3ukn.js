...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  filteredProducts = searchValue.length > 0 ? data.products.filter((val) => val.name.toLowerCase().includes(searchValue.toLowerCase())) : data.products?.slice(0, 9);
  $$unsubscribe_showLoader();
  $$unsubscribe_navigating();
  return `<div class="flex flex-col md:flex-row items-start gap-2 md:gap-6"><main class="${"md:" + escape(data.theme.card, true) + " w-full"}"><div class="${"flex items-start md:p-4 gap-6 " + escape(data.theme.bgColor, true) + " md:border-b " + escape(data.theme.border, true) + " rounded-t-lg md:sticky top-[4.5rem] z-10 bg-opacity-75 backdrop-blur-md"}"><img${add_attribute("src", data.game.image, 0)} class="w-[72px] aspect-square rounded-lg" alt=""> <div><p class="text-sm opacity-50" data-svelte-h="svelte-1g9ct0t">Topup Games</p> <h4 class="text-3xl font-bold">${escape(data.game.name)}</h4></div></div> <form method="post" data-sveltekit-keepfocus data-sveltekit-noscroll class="space-y-8 md:p-4 py-6"${add_attribute("this", submitForm, 0)}><div class="w-full flex items-start gap-6" id="inputIdpel"><h1 class="font-bold text-4xl opacity-75 w-[40px] text-right hidden md:block" data-svelte-h="svelte-11eubrv">1.</h1> <div class="space-y-4 w-full"><div><h4 class="font-medium"><span class="md:hidden" data-svelte-h="svelte-1j8ht2n">1. </span>Masukkan Game ID ${escape(data.game.name)} Anda</h4> <p class="text-sm opacity-50" data-svelte-h="svelte-ud42bt">Silahkan anda mengisi dengan game ID anda, contoh : 213123123</p></div> ${data.game.fields && data.game.fields?.length > 0 ? `<div class="${"grid " + escape(
