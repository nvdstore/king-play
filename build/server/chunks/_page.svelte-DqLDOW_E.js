...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<form class="space-y-6"><header class="space-y-1.5 text-center md:text-left" data-svelte-h="svelte-ov9di7"><h4 class="text-2xl font-medium">Lupa Kata Sandi</h4> <p class="text-sm">Masukkan alamat Email Anda, dan kami akan mengirimkan link untuk Reset Kata Sandi Anda.</p></header> <section class="space-y-4"><div class="input-group w-full"><label for="email" class="input-label" data-svelte-h="svelte-rx7s2m">Email</label> <input name="email"${add_attribute("class", data.theme.input, 0)} type="email" placeholder="Masukkan Email Anda"${add_attribute("value", email, 0)}></div> <div class="input-group w-full"><label for="confirm-email" class="input-label" data-svelte-h="svelte-1ontm1s">Konfirmasi Email</label> <input name="confirm-email"${add_attribute("class", data.theme.input, 0)} type="email" placeholder="Konfirmasi Email Anda"${add_attribute("value", confirmEmail, 0)}></div> <button class="btn btn-primary w-full" type="submit" data-svelte-h="svelte-p95eca">Kirim Link</button> <a href="/auth/login"${add_attribute("class", data.theme.button, 0)}>Masuk</a></section></form>`;
});

export { Page as default };
//# sourceMappingURL=_page.svelte-DqLDOW_E.js.map
